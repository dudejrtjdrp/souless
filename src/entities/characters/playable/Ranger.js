...
export default class Ranger extends CharacterBase {
  constructor(scene, x, y, options = {}) {
    const config = CharacterDataAdapter.buildConfig('ranger', options);

    //  수정: 전달받은 x, y를 그대로 사용 (포탈 위치 강제 제거)
    super(scene, x, y, config);

    // SkillSystem 초기화
    const skillsData = CharacterDataAdapter.getSkillsData('ranger');
  ...
  static preload(scene) {
    scene.load.spritesheet('ranger', '/assets/characters/ranger_spritesheet.png', {
      frameWidth: 288,
      frameHeight: 128,
    });
  }
  ...
  getAnimationConfig() {
    return CharacterDataAdapter.getAnimationConfig('ranger');
  }
